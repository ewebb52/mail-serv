...
if [ "$#" -ne 1 ]; then
    echo "usage: ./cert-authority-2c.sh <private-key-filename>"
    exit
fi
echo $#
...
cp "${BASE}/init/2c-openssl.cnf" "${ROOT}/openssl.cnf"
...
cp "${BASE}/init/2c-intra-openssl.cnf" "${INTERMEDIATE}/openssl.cnf"
